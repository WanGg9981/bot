...
let handler = async (m, { conn }) => {
    ...
    try {
        ...
        if (/webp/.test(mime)) {
            ...
            if (!img) throw `Reply sticker dengan commands *#s*`
            stiker = await sticker5(img, false, packname, author)
        } else if (/image/.test(mime)) {
            let img = await q.download()
            if (!img) throw `Reply gambar dengan commands *#s*`
            stiker = await sticker5(img, false, packname, author)
        } else if (/video/.test(mime)) {
            if ((q.msg || q).seconds > 11) return m.reply('Maksimum 10 saat')
            let img = await q.download()
            if (!img) throw `Reply video dengan commands *#s*`
            stiker = await sticker5(img, false, packname, author)
        } else if (m.quoted.text) {
            if (isUrl(m.quoted.text)) stiker = await sticker(false, m.quoted.text, packname, author)
            else throw 'Link tidak sah'
        }
    } catch (e) {
        throw e
    }
